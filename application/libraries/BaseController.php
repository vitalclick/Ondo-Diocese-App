...
class BaseController extends CI_Controller
{
		...
		if (array_key_exists('Authorization', $headers)) {
			...
			if ($jwt) {
				try {
					...
					echo json_encode(array("errors" => true, "status" => "error", "message" => "Unauthorized"));
					exit;
				}
			} else {
				/*
						 ...
						 */
				http_response_code(401);
				echo json_encode(array("errors" => true, "status" => "error",  "message" => "No request token found. Sign in and try again."));
				exit;
			}
		} else {
			/*
	...
	public function get_data()
	{
		...
		if (isset($data->email)) {
			$data->email = filter_var($this->cleanup($data->email), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
		if (isset($data->name)) {
		...
		if (isset($data->token)) {
			$data->token = filter_var($this->cleanup($data->token), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
		if (isset($data->plan)) {
			$data->plan = filter_var($this->cleanup($data->plan), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
		if (isset($data->date)) {
			$data->date = filter_var($this->cleanup($data->date), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		}
