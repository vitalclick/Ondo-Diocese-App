    ...
    */
    public function resendVerificationMail(){
     ...
     if(!empty($data)){
        ...
        $r = explode('/', $r);
				$email = isset($r[3])?filter_var(urldecode($r[3]), FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
				$activation_id = isset($r[2])?filter_var($r[2], FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
      ...
      redirect('androidUsers');
		...
		public function deleteaccount(){
			...
			if(!empty($data)){
				  $email = isset($data->email)?filter_var($data->email, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):"";
          if($email!=""){
             $this->account_model->deleteaccount($email);
				 }
			 }
		...
		public function counsellingrequest(){
			...
			$availabledates =$this->input->post('availabledates');
			$content = $this->input->post('content');
			$this->sendCounsellingMail($email,$name,$content,$availabledates);
			$this->sendUserCounsellingMail($email,$name,$content,$availabledates);
		  $this->response("ok","Mail Sent");
		}
}
