...
class Chat extends BaseController
{
  ...
  {
    $date = time();
    $chat_id = null !== $this->input->post('chat_id') ? filter_var($this->input->post('chat_id'), FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : 0;
    $sender = null !== $this->input->post('sender') ? filter_var($this->input->post('sender'), FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
    $recipient = null !== $this->input->post('recipient') ? filter_var($this->input->post('recipient'), FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
    $msg_reciept = null !== $this->input->post('msg_reciept') ? filter_var($this->input->post('msg_reciept'), FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : time();
    $msg_owner = null !== $this->input->post('msg_owner') ? filter_var($this->input->post('msg_owner'), FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
    $message = null !== $this->input->post('content') ? filter_var($this->input->post('content'), FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
    if ($chat_id == 0) {
      //first we check if a chat have been initiated before
      //between both users and get the chat id
