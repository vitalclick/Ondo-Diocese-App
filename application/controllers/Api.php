...
class Api extends BaseController
{
	...
	function discover()
	{
		...
		//$last_seen_event = isset($data->last_seen_event)?filter_var($data->last_seen_event, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH):0;
		...
		$paypal_client = $this->settings_model->getPaypalClient();
		$paypal_secret = $this->settings_model->getPaypalSecret();
		$slider_media = $this->media_model->fetchRandom($email);

		echo json_encode(array(
			"status" => "ok", "slider_media" => $slider_media, "livestream" => $livestreams, "facebook_page" => $facebook_page, "youtube_page" => $youtube_page, "twitter_page" => $twitter_page, "instagram_page" => $instagram_page, "ads_interval" => $ads_interval, "inbox" => $inbox, "website_url" => $website_url, "image_one" => $image_one, "image_two" => $image_two, "image_three" => $image_three, "image_four" => $image_four, "image_five" => $image_five, "image_six" => $image_six, "image_seven" => $image_seven, "image_eight" => $image_eight, "paypal_client" => $paypal_client, "paypal_secret" => $paypal_secret, "events" => $events, "radios" => $radios
		));
	}
	...
	function fetch_events()
	{
		...
		$this->load->model('events_model');
		$results = $this->events_model->fetchMonthsEvents($month, $year);
		echo json_encode(array("status" => "ok", "events" => $results));
	}

	//fetch albums
	function fetch_prayerpoints()
	{
		$data = $this->get_data();
		$date = isset($data->date) ? filter_var($data->date, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : date("Y-m-d");
		$this->load->model('prayers_model');
		$results = $this->prayers_model->fetchPrayerPoints(date('Y-m-d', strtotime($date)));
		echo json_encode(array("status" => "ok", "prayers" => $results));
	}
	...
	function fetch_books()
	{
		...
		$results = $this->books_model->fetch_ebooks($page);
	...
	public function update_media_total_views()
	{
		...
		$this->load->model('media_model');
		if (!empty($data)) {
			$media = isset($data->media) ? filter_var($data->media, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
	...
	public function getmediatotallikesandcommentsviews()
	{
		...
		echo json_encode(array(
			"status" => 'ok', "total_likes" => $total_likes, "total_comments" => $total_comments, "total_views" => $total_views
		));
	}
	...
	function storeFcmToken()
	{
		...
		echo json_encode(array(
			"status" => $this->fcm_model->status, "msg" => $this->fcm_model->message
		));
	}
	...
	function send_feedback()
	{
		...
		if (!empty($data)) {
			...
			$email = isset($data->email) ? filter_var($data->email, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$phone = isset($data->phone) ? filter_var($data->phone, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
	...
	public function saveDonation()
	{
		...
		if (!empty($data)) {
			...
			$method = isset($data->method) ? filter_var($data->method, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$email = isset($data->email) ? filter_var($data->email, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$name = isset($data->name) ? filter_var($data->name, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
	...
	public function purchasebook()
	{
		...
		//var_dump($data); die;
		if (!empty($data)) {
			$bookid = isset($data->bookid) ? filter_var($data->bookid, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$booktitle = isset($data->bookname) ? filter_var($data->bookname, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$email = isset($data->email) ? filter_var($data->email, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$name = isset($data->name) ? filter_var($data->name, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$amount = isset($data->amount) ? filter_var($data->amount, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : 0;
			$reference = isset($data->reference) ? filter_var($data->reference, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";
			$shipping = isset($data->shipping) ? filter_var($data->shipping, FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_HIGH) : "";

			$this->load->model('books_model');
			$pay_ref['email'] = $email;
			$pay_ref['name'] = $name;
			$pay_ref['shipping'] = $shipping;
			$pay_ref['booktitle'] = $booktitle;
			$pay_ref['reference'] = $reference;
			$pay_ref['amount'] = $amount;
			$pay_ref['bookid'] = $bookid;
			$this->books_model->recordpurchase($pay_ref);
			$this->sendpurchasebookemail($email, $name, $booktitle, $reference, $shipping);
			echo json_encode(array("status" => "ok"));
			exit;
		} else {
			echo json_encode(array("status" => "error", "message" => "No data found for this transaction"));
